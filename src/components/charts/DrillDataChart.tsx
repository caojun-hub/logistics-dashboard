...
import type { DrillData } from '@/lib/data';
...
export function DrillDataChart({ data, district, weekDate }: Props) {
  const result = getDrillBizSummary(data, district, weekDate);
  ...
  const option = {
    ...
    xAxis: {
      ...
      data: result.flatMap(r => r.bizareas.map(a => a.name)),
      axisLabel: { fontSize: 9, rotate: 30 },
    },
    yAxis: {
      type: 'value' as const,
      name: '推单量',
      axisLabel: { fontSize: 10, formatter: (v: number) => v >= 10000 ? `${v / 10000}万` : v.toLocaleString() },
    },
